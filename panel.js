const icons = {
    ...
    narration: '<path d="M12 6C9 4 5 4 3 5v14c3-1 6-1 9 1m0-14c3-2 7-2 9-1v14c-3-1-6-1-9 1V6Z"/>',
...
const pages = {
    ...
    connection: ['连接与播放', '连接 Breeze，按你的习惯聆听故事。'],
};
      ...
      <div class="workspace">
        ...
        <main class="page-scroll">
          <section data-page="characters" id="page-characters" aria-label="角色与音色">
            ...
            <section class="surface narrator-card" aria-labelledby="narrator-heading">
              <div class="narrator-heading"><span class="section-icon">${icon('narration')}</span><div><h3 id="narrator-heading">旁白声音</h3><p>选择音色与获取方式，按正文顺序穿插朗读旁白；默认播放时获取。</p></div><span class="editor-chip">随当前聊天保存</span></div>
              <div class="narrator-fields">
                <div class="field narrator-fetch-field"><label for="breeze-narrator-fetch-mode">旁白获取方式</label><select id="breeze-narrator-fetch-mode" data-narrator-fetch-mode aria-describedby="narrator-fetch-help"><option value="off">不获取</option><option value="playback" selected>播放时获取</option><option value="auto">自动获取</option></select><small id="narrator-fetch-help" data-narrator-fetch-help>全部播放时随播放进度生成旁白，仅生成语音时跳过旁白。</small></div>
                <div class="field"><label for="breeze-narrator-voice">旁白音色</label><div class="narrator-voice-control"><select id="breeze-narrator-voice" data-narrator-voice aria-describedby="narrator-status"><option value="">未选择旁白音色</option></select><button class="preview-button" type="button" data-narrator-preview aria-label="试听旁白音色" title="试听所选音色" disabled>${icon('play')} 试听</button></div></div>
                ...
                <label class="field narrator-target-field" for="breeze-narrator-target-chars"><span>旁白切片目标字数<small id="narrator-target-help">跨换行累计，目标内按最后完整句子切分；单句过长保留整句。普通引号内容也由旁白朗读。</small></span><input id="breeze-narrator-target-chars" data-narrator-target-chars type="number" min="20" max="1000" step="1" value="100" required aria-describedby="narrator-target-help"></label>
              </div>
          ...
          <section data-page="prompt" id="page-prompt" aria-label="预设注入" hidden>
            ...
            <div class="prompt-guidance-grid">
              <section class="surface prompt-guidance-card" aria-labelledby="vocal-events-heading">
                ...
                <label class="field" for="breeze-vocal-events">允许的语气词<textarea id="breeze-vocal-events" data-vocal-events rows="8" spellcheck="false" placeholder="[笑]&#10;(laugh)&#10;(breathy sigh)" aria-describedby="vocal-events-help vocal-events-status"></textarea><small id="vocal-events-help">默认中文使用 []，英文使用 ()。每行一项，也可用逗号分隔；相邻标签自动拆分。自定义项保留括号，省略时补为 []。</small></label>
          ...
          <section data-page="connection" id="page-connection" aria-label="连接与播放" hidden>
            ...
            <div class="settings-columns"><div class="surface setting-card"><div class="card-heading"><span class="section-icon">${icon('play')}</span><div><h3>全文播放</h3><p>选择从文字到声音的方式。</p></div></div><label class="switch-row"><span><strong>Breeze 总开关</strong><small>关闭后停止聊天语音生成与播放，并移除本插件的预设注入；原有设置会保留。</small></span><input data-setting="enabled" type="checkbox" role="switch"><span class="switch" aria-hidden="true"></span></label><label class="switch-row"><span><strong>自动生成语音</strong><small>为已绑定角色与旁白准备语音；开启读取流式文本后，完整内容到达即可生成。</small></span><input data-setting="autoGenerate" type="checkbox" role="switch"><span class="switch" aria-hidden="true"></span></label><label class="switch-row"><span><strong>自动播放全文</strong><small>按正文顺序生成并播放。绑定旁白音色后，会在对白之间朗读旁白；开启读取流式文本后，无需等待整条回复完成。</small></span><input data-setting="autoPlay" type="checkbox" role="switch"><span class="switch" aria-hidden="true"></span></label><label class="switch-row"><span><strong>读取流式文本</strong><small>完整对白或旁白段落到达时，按自动生成／自动播放设置处理；关闭则等待整条回复完成。不完整标签仍实时隐藏。此设置与音频流式开关独立。</small></span><input data-setting="readStreamingText" type="checkbox" role="switch"><span class="switch" aria-hidden="true"></span></label><label class="switch-row"><span><strong>流式生成与播放</strong><small>音频边合成边播放，完成后保留完整音频供重播。与读取流式文本独立；仅预生成时仍保存完整音频。</small></span><input data-setting="streaming" type="checkbox" role="switch"><span class="switch" aria-hidden="true"></span></label><label class="switch-row"><span><strong>显示为对白与气泡</strong><small>开启时隐藏标签与已知语气词，显示完整对白；关闭时显示原始标签。</small></span><input data-setting="hideTags" type="checkbox" role="switch"><span class="switch" aria-hidden="true"></span></label><label class="volume-field"><span>播放音量 <output data-volume-value>80%</output></span><input data-setting="volume" type="range" min="0" max="1" step="0.05"></label></div><div class="settings-side"><div class="surface setting-card"><div class="card-heading"><span class="section-icon">${icon('voices')}</span><div><h3>合成参数</h3><p>用于对白、旁白与候选声音的生成。</p></div></div><label class="field">指令强度 · CFG<input data-setting="cfgScale" type="number" min="0.1" max="20" step="0.1"><small>控制模型对声音指令的遵循强度。</small></label><label class="field">随机种子<input data-setting="seed" type="number" min="0" max="4294967295" step="1"><small>设计多个候选时，种子依次递增。</small></label></div><div class="playback-help"><h3>播放小提示</h3><p>浏览器可能要求先手动点击一次播放。右键点击气泡，可清除该句缓存引用并在下次重新生成。</p><button class="button stop-button" type="button" data-stop>${icon('stop')} 停止队列与播放</button></div></div></div>
            <section class="surface text-filter-card" aria-labelledby="text-filter-heading">
              <div class="guidance-heading"><h3 id="text-filter-heading">文本过滤</h3><span class="editor-chip">所有聊天共用</span></div>
              <p id="text-filter-help" class="guidance-description">合成角色对白与旁白时，将匹配的完整文字替换为空格。仅影响送去合成的文本，原文、可读对白和预设注入保持不变。</p>
              <div data-text-filter-editor aria-describedby="text-filter-help"></div>
            </section>
          </section>
        </main>
