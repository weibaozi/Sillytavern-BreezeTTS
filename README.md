# SillyTavern Breeze 角色语音 · 0.7.0-tag-render.8（实验分支）
...
2. 在本实验目录执行安装脚本；需要显式添加 `-InstallExperimental`。脚本先备份当前同名扩展，再替换到：
   ...
   目录下应直接包含 `manifest.json`、`index.js`、`dialogue-render.js`、`automatic-queue.js`、`panel.js`、`panel.css`、`stream-player.js`、`extra-prompts.js` 等文件。其他酒馆用户使用其自己的用户目录，可通过 `-UserHandle` 指定。实验包为 `dist/sillytavern-breeze-0.7.0-tag-render.8.zip`。
...
消息底部控件独立于酒馆通用按钮宽度，桌面横排、窄屏整体换行。「暂停／继续」保留当前进度和剩余队列，生成中也可暂停等待；「停止」结束当前播放并清空队列。「播放本条」和「自动播放全文」按原文顺序穿插旁白及已绑定角色对白；单个对白气泡仍只播放该句。仅开启自动生成时也会准备旁白音频，已有缓存继续复用。旁白正文外观保持原样，只有旁白的消息也可整条播放。

旁白从正文文字提取，排除 TTSVoice、引号内台词、代码、图片、正文前后模块及咪咪吐槽、摘要等，不会把历史双份对白的引号副本再当旁白读出。旁白切片默认目标 100 字，可在旁白声音卡中设置 20–1000 字。同段按目标内最后一个完整句末合并切片，计数以可朗读字符（含标点）为准；单句超过目标时保留整句，仍受 5000 字请求上限保护。换段和对白边界立即切开。流式读取在后续文字确定当前片段已无法继续容纳下一句时入队，结束时补读尾段；停止、切换聊天或修改旁白设置会取消旧队列。续写自动播放只读新增部分。
