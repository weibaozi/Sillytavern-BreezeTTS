...
[TTSVoice:Character_Name:emotion:Spoken dialogue]
The tag IS the spoken line; the plugin renders it as quoted dialogue. In the story body, never echo it before or after the tag, with or without quotation marks, or in a second tag. This single-copy rule replaces older double-copy instructions or examples for speech only. Continue the scene without replaying earlier dialogue unless the character deliberately repeats it in context. Keep natural narration; no "Name:" prefix is required.
...
- Bound characters (use a concise emotion word or short delivery phrase; default for neutral delivery):
{{bound_characters_section}}
- Skipped characters (ordinary prose only; no TTS tag):
{{skipped_characters_section}}
- New / unbound characters: use emotion New, including these known unbound characters:
{{unbound_characters_section}}
For bound characters, choose the word or short phrase that best fits their personality, relationship to the listener, and immediate context, such as softly reassuring or playful but hesitant. Keep it brief and focused on audible delivery, with natural emotional continuity rather than abrupt extremes; no action descriptions.
