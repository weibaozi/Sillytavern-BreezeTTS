# Breeze 角色语音 · 0.7.0-tag-render.4（实验版）
...
脚本需要显式实验开关，先备份再替换同名扩展。包名为 `dist/sillytavern-breeze-0.7.0-tag-render.4.zip`；部署应包含 `dialogue-render.js`、`automatic-queue.js` 等全部文件，刷新酒馆生效，从 0.6.1 试用无需重启已有配套后端。
